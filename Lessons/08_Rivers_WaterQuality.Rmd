...
NeuseWQFlow_processed <- NeuseWQ_processed %>%
  full_join(., NeuseFlow_processed) %>% #full join b/c keep daily flow measurements with water quality measurements. 
...
plot(lm(data = NeuseWQFlow_processed, log10(FecalColiform_cfu.100mL) ~ log10(Flow_cfs))) #check assumptions for lm
...
ggplot(NeuseWQFlow_processed, aes(x = HydrologicCondition, y = FecalColiform_cfu.100mL)) +
  geom_boxplot(outlier.shape = NA) +
  ylim(0,300) +
  scale_x_discrete() +
  geom_jitter(alpha = 0.5, color = "olivedrab4") +
  labs(x = "Hydrologic Condition", y = "Fecal Coliform (cfu/100ml)")

ggplot(NeuseWQFlow_processed, aes(x = HydrologicCondition, y = SpC_uS.cm)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(alpha = 0.5, color = "violetred") +
  labs(x = "Hydrologic Condition", y = expression("Specific Conductivity ("*mu*"S/cm)")) +
