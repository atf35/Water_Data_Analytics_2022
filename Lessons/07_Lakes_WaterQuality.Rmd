...
# Additional help resources for the LAGOS database (metadata): 
...
LAGOSMN <- LAGOSjoined %>% 
  ...
  filter(state_name == "Minnesota", 
         month %in% c(6:9),
         year >= 1990,
         hu4 %in% c("0401", "0701", "0702", "0703", "0704", "0902"))

ggplot(LAGOSMN, aes(x = tp, y = chla)) +
  geom_point(alpha = 0.5) +
  geom_smooth(method = lm) +
  facet_wrap(vars(hu4)) +
  scale_y_log10() +
  scale_x_log10() +
  labs(x = "Total Phosphorus", y = "Chlorophyll a")


lm.model <- lm(data = LAGOSMN, log10(chla) ~ log10(tp) * hu4) # * changes slope and intercept, + change just the intercept
summary(lm.model)
...
LAGOScovariates <- LAGOSjoined %>%
  mutate(percentdeveloped = buffer500m_nlcd2011_pct_22 + buffer500m_nlcd2011_pct_23 + buffer500m_nlcd2011_pct_24,#forested landtype
         percentforest = buffer500m_nlcd2011_pct_41 + buffer500m_nlcd2011_ha_42 + buffer500m_nlcd2011_ha_43,#developed landtype
         percentrowcrops = buffer500m_nlcd2011_pct_82) %>% #agricultural landtype 
...
LAGOScorr <- cor(LAGOScovariates, use = "complete.obs") #restrict to complete observations in the 11 variables
