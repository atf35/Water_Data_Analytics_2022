...
> There may be some seasonality. It is difficult to see if there is a trend at this point. Accounting for seasonality and noise will help detect the trend and forcast future conditions. There may be an increase in the number of occurances of peaks. From other classes, we have seen that this river does not display stationarity, and is seeing increasing variation over time. We may be able to see a trend if we log the y-axis. 
...
> It would not be appropriate to use a linear regression to model this time series at this point. We would need to remove seasonality and variation. Trends in population growth? Aggregated to annual time scale can remove the impacts of wild swings in the data.  
...
Neuse_ts <- ts(NeuseFlow[[4]], frequency = 365) #data needs to be equally spaced within the time series!! double brackets around the 4 is specific to the ts() function.
...
```{r}
?stl #loess = moving average approach 

# Generate the decomposition
Neuse_Decomposed <- stl(Neuse_ts, s.window = "periodic")

# Visualize the decomposed series. 
plot(Neuse_Decomposed) #bar on the sides shows the scale relative to other components
...
> Trend smooths out the extreme events in the data. The trend also drops off at the end of the data set, likely because there is no observations at the end of the dataset and it is a average of a moving window. We would also expect that a moving average may go negative, which is not actually feasible. In this case, there are no negative trends. 
...
* **Piecewise constant**: "nearest neighbor" interpolation. We would see a bunch of horizontal lines across the dataset.  The value will be the nearest neighbor until the halfway point. 
* **Linear**: "connect the dots" 
* **Spline**: quadratic formulas 
...
NeuseCond_processed <- NeuseCond %>%
  ...
  mutate(Year = year(Date),
         Month = month(Date),
         Date_monthrounded = floor_date(Date, "month")) %>% #floor_date() --> rounds date down
  arrange(Date)

NeuseCond_monthly <- left_join(Months, NeuseCond_processed)

# Generate monthly values from October 1980 to December 2021
linearinterpolation <- as.data.frame(approx(NeuseCond_monthly$Conductivity, n = 566, method = "linear")) #approx() --> approximates data, n= --> number of rows in the dataset. this function does not do spline! 
NeuseCond_monthly$Conductivity <- linearinterpolation$y #replace conductivity column with the y column in the interpolation dataframe
...
conductivity_decomp <- stl(conductivity_ts, s.window = "periodic")

plot(conductivity_decomp)

conductivity_components <- as.data.frame(conductivity_decomp$time.series[,1:3])
conductivity_components <- mutate(conductivity_components,
                                  Observed = NeuseCond_monthly$Conductivity,
                                  Date = NeuseCond_monthly$Date_monthrounded)
