...
* number of tributaries/ stream order
...
**Stormflow** (a.k.a. "quickflow") is attributed to overland flow and shallow subsurface flow that deliver water quickly to a stream or river of interest. **Baseflow** is attributed to slower flow pathways, including but not limited to groundwater. Snowmelt is sometimes considered baseflow.
...
EnoDischarge <- EnoDischarge %>%
  mutate(Baseflow_lfstat = baseflow(Discharge), #uses beginning and end of dataset to calibrate, so it will not calculate baseflow or stormflow for these periods (total 24 days)
...
# plot baseflow and total flow: Lower base flow with lower water table. There is high variability in the Eno River baseflow. Total magnitudes of baseflow and total flow are quite different.
ggplot(EnoDischarge, aes(x = Date, y = Discharge)) +
...
# Note: this function creates two columns which can be joined onto the original dataset. This function will calculate baseflow and storm flow at the same time, without us having to calculate stormflow manually. However, you need to combine it back to the original dataset because the output will only be baseflow and quickflow.
...
ggplot(EnoDischarge, aes(x = Baseflow_lfstat, y = bt)) +
  ...
  geom_abline(slope = 1, intercept = 0, color = "darkred") #1:1 line that shows whether or not the two values are exactly the same. Plot shows that they are related, but not exactly the same. They are more similar at smaller flows. In some cases, one baseflow is a range of baseflows using the other method. Departures at higher flows represent storm events (curvilinear areas). 

# How would we test that statistically? [add code here]
summary(lm(bt ~ Baseflow_lfstat, data = EnoDischarge))
...
#difference in baseflow results from differential smoothing across stormflow events. Storm-driven increases in baseflow due to water table recharging.
...
EnoDischarge_summary <- EnoDischarge %>%
  group_by(WaterYear) %>% #run summary statistics on wateryear
  summarise(Discharge.acft.yr = sum(Discharge)*723.968, # what is this factor? cfs-->acre-ft/yr
            Baseflow.acft.yr = sum(bt)*723.968,
            Stormflow.acft.yr = sum(qft)*723.968,
            Prop.bf = Baseflow.acft.yr/Discharge.acft.yr, #proportions
            Prop.sf = Stormflow.acft.yr/Discharge.acft.yr) %>%
 mutate_if(is.numeric, round, 2) # if a column is numeric, round it to two decimal places

#high stormflow --> higher baseflow
ggplot(EnoDischarge_summary, aes(x = WaterYear, y = Discharge.acft.yr)) +
  geom_line() +
  geom_line(aes(y = Baseflow.acft.yr), lty = 2)

#proportion over time using columns/bars
...
>This area is located around the High Plains Aquifer (Ogallawa Aquifer). Pumping from this aquifer resulted in non-perennial flows in Dodge City, where the Arkansas River has become a losing reach.
...
```{r}
DodgeCityDischarge <- readNWISdv(siteNumbers = "07139500",
                     parameterCd = "00060", # discharge (ft3/s)
                     startDate = "1960-1-01", 
                     endDate = "2005-12-31")

names(DodgeCityDischarge)[4:5] <- c("Discharge", "Approval.Code")

DodgeCityDischarge_bf <- BaseflowSeparation(DodgeCityDischarge$Discharge)
DodgeCityDischarge <- cbind(DodgeCityDischarge, DodgeCityDischarge_bf)


ggplot(DodgeCityDischarge, aes(x = Date, y = Discharge)) +
  geom_line(color = "gray") +
  geom_line(aes(y = bt)) +
  scale_y_log10() +
  labs(x = "Year", y = "Discharge (cfs)")
