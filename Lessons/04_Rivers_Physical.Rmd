...
* Higher variability in flow (flashy)
* Shifts in peak discharge, timing of snowmelt
* Types of discharge change
* Changes in evapotranspiration due to temperature increases 
...
# install.packages("lfstat") #low flow statistics
...
EnoDischarge <- readNWISdv(siteNumbers = "02085070",
                     ...
                     startDate = "", 
                     endDate = "2021-09-30") # "the water year" starts on the first day of October and ends on the last day of September. This captures the full snow season instead of splitting it in half with Jan and Dec. 

names(EnoDischarge)[4:5] <- c("Discharge", "Approval.Code")

attr(EnoDischarge, "variableInfo") # mean daily discharge
attr(EnoDischarge, "siteInfo") # geographic context of the gage
...
#event-based discharge peaks. some seasonablity but not as much as in snowpack affected rivers. 
```
...
# different regions of the world have different water years. this is why you need to specify the origin. need to filter out 1963 in the water year because otherwise the 1963 water year will only be 30 days long. R always computes the water year as a factor. this is why it is in quotations.
...
# need to specify that a factor is a character first before you change it to numeric. if you try to change a factor just to a number, then it will rank the years (e.g. 1963 will be 1).

# Calculate Recurrence Intervals for 30 years. group_by water year to analyze the whole water year at the same time (one estimate per water year). summarise the max discharge per water year. this is similar to a pivot table.
EnoRecurrence <- 
  EnoDischarge %>%
...
# using summary statistics...intercept (coeff. 1) + slope (coeff. 2) * recurrence interval
```
...
ggplot(EnoRecurrence, aes(x = RecurrenceInterval, y = PeakDischarge)) +
  ...
  scale_x_log10()
# less frequent events are higher. lowest discharges overlap. discharges at higher recurrence intervals have higher peak discharges.
...
#slope in later years has greatly increased!
...
#100 and 500 year floods are much greater than before
...
ggplot(SaladoDischarge, aes(x = Date, y = Discharge)) +
  geom_line() +
  labs(x = "Year", y = "Discharge (cfs)")
# flashy, event based discharge (random high, fast peaks that are not seasonal). more small-medium sized events in recent years. possible data gaps. highs are higher. Urbanization and channelization of flow. 
...
ggplot(GreenDischarge, aes(x = Date, y = Discharge)) +
  geom_line() +
  labs(x = "Year", y = "Discharge (cfs)")
# dam was built for flood control! you can see the drop in discharge. 
