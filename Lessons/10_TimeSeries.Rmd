...
> Can think about changes in policy or dam installations/removals. 
...
> 10-years is considered the minimum threshold for considering monotonic trend analysis. 
...
> aggregation: account for gaps (e.g. if you have daily data, but dont' have data for a few days, you can aggregate them by month). Some time series analyses require less autocorrelation. Aggregation can reduce temporal autocorrelation (e.g. daily data will have autocorrelation with the previous day, but Jan/Feb/Mar may not be as autocorrelated).

> subsampling: subset a sample of your data to build a model with, and then run your model on the rest of your data. Can also subset at a different frequency (e.g. you have daily data, but know it is autocorrelated, so you only take the values at every 14 days). 

> interpolation: use surrounding data to fill in the gaps. 
...
* **Seasonal Mann-Kendall**: seasonality, non-parametric, no temporal autocorelation, identical distribution (equally spaced time steps). Function: `smk.test` (package: trend)
...
```{r}
Condplot <-
ggplot(drop_na(NeuseCond_monthly), aes(x = as.factor(Month), y = Conductivity)) +
  geom_boxplot() +
  labs(x = "", y = expression("Conductivity "*mu*"s/cm"), title = "Neuse River")

Flowplot <-
ggplot(NeuseFlow_monthly, aes(x = as.factor(Month), y = Discharge_acftmo)) +
  geom_boxplot() +
  labs(x = "Month", y = "Discharge (acft/mo)")

plot_grid(Condplot, Flowplot, ncol = 1, align = "hv")
```

> We see higher conductivity in the summer months and lower conductivity in the winter months. We see the opposite for flow, with lower discharge in the summer months and higher discharge in the winter months. 
...
NeuseCond_timeseries <- ts(NeuseCond_monthly$Conductivity, frequency = 12,
                           start = c(1980, 10, 1), end = c(2021, 12, 1)) #interpolated up to monthly data, so the day doesnt matter (just a place holder)
# Run SMK test
NeuseCond_trend <- smk.test(NeuseCond_timeseries)

# Inspect results
NeuseCond_trend #positive z score --> positive trend over time. S --> sum of differences over time. 
...
> We see a positive trend in conductivity over time with a positive z score and a p-value of <0.05. Conductivity is a proxy for salinity, so we see that the Neuse River is becoming more salinated over time. The summary function for the SMK test will give us the significance of the trends for each month. We see that increased salinity occurs in the winter months. 
...
NeuseFlow_trend
summary(NeuseFlow_trend)

```

What would we conclude based on these findings? 

> With a positive z score and a p-value < 0.05, we see that there is a monotonic increase in discharge over time. Our summary shows that the increase in discharge is occuring in the fall months, which is the typical hurricane season in North Carolina. 

> Since we do not see an increase in discharge in the winter months, but we see an increase in conductivity, we can conclude that there is a significant input of material into the Neuse River during the winter months that increases the salintiy (for example, road salts or construction material). 
...
adf.test(NeuseFlow_timeseries, alternative = "stationary") #test requires stationary data, however, this may not be the case. Null hypothesis is that the data are NOT stationary, so the p-value < 0.05 indicates that the data ARE stationary. 
```
...
acf(NeuseFlow_timeseries) #autocorrelation. first month will always be autocorrelated. Autocorrelation decays over time and we can see seasonality in our data. 
pacf(NeuseFlow_timeseries) #moving average. first month will always be partially autocorrelated. decays over time. see seasonality. 
...
Neuseprediction <- predict(fit, n.ahead = 10*12) #n.head --> run 10 years of 12 months each

# plot future predictions
ts.plot(NeuseFlow_timeseries, Neuseprediction$pred, lty = c(1, 3)) #data in solid, prediction in dotted lines. 

```

How do future predictions compare to the past? Why might the forecast be lacking in complexity?

> There is a lot of variation in actual precipitation over time. Our prediction does not show this variation. This autocorrelation test is maybe not the best option for this dataset. 
