...
## Set ActivityStartDate as date. (#3)
NCdata_processed$ActivityStartDate <- as.Date(NCdata_processed$ActivityStartDate, format = "%Y-%m-%d")

## Wrangle new processed dataset (#4)
NCdata_processed_wrangled <- NCdata_processed %>% 
  select("OrganizationIdentifier", "OrganizationFormalName", "ActivityStartDate", 
"ActivityConductingOrganizationText", "MonitoringLocationIdentifier", "Depth_m", "ResultMeasureValue", "Variable") %>% 
  group_by(OrganizationIdentifier, OrganizationFormalName, ActivityStartDate, 
ActivityConductingOrganizationText, MonitoringLocationIdentifier, Depth_m, Variable) %>% 
...
## Chlorophyll-TP stressor-response relationship (#5)
ggplot(NCdata_processed_wrangled, aes(x = TP_mgL, y = Chla_ugL)) +
  geom_point(alpha = 0.5) +
  geom_smooth(method = lm, se = FALSE, color = "darkred") +
  scale_x_log10() +
  scale_y_log10() +
  labs(x = expression("Total Phosphorus (mg/L)"),
...
> I expect the Piedmont to have a weak total phosphorus-chlorophyll a response because agricultural inputs of phosphorus in this region is extremely high and there is an excess of P in Piedmont systems. I expect NC mountains to have a strong TP-Chla relationship since P is typically a limiting nutient in mountainous regions. Comparatively, the Coastal Plain is expected to have a low stressor-response relationship because agricultural runoff and sediment runoff accumulate in this region and P is typically in excess. I anticipate that reservoirs would have excess P and thus would have a weaker stressor-response relationship compared to natural lakes. 